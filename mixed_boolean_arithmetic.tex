...
\thm{Definition 1} {Let $n,s,t \in \N$, where $n$ is the number of bits in a bit vector, 
...
$A = (v_{i,j})_{2^t \times s}$ be the $\B$ matrix of truth tables over ($\Z/2^n\Z$).
...
\pf{
    ...
    of $y_j$. Then we have the following:
    \begin{align*}
        e = \sum_{j=0}^{s-1}
    \end{align*}
}
